import { getFirestore, doc, getDoc, setDoc, deleteDoc, query, where, getDocs, collection, orderBy, onSnapshot, serverTimestamp } from "https://www.gstatic.com/firebasejs/9.4.0/firebase-firestore.js";
...
function addNotification(crush) {
  ...
  notification.textContent = "You have matched with: " + crushData[0];

  notification.addEventListener('click', async function() {
    const messagesList = document.getElementById('messagesList');
    messagesList.scrollTop = messagesList.scrollHeight;
    await showConversation(crushData);
  });

  notificationBox.appendChild(notification);
}
async function showConversation(crush) {
  const crushEmail = crush[1];
  const messagesList = document.getElementById('messagesList');
  messagesList.innerHTML = '';

  const convoId = crush[2];
  const crushName = crush[0];
  ...
  if (convoId) {
    ...
    onSnapshot(q, (snapshot) => {
      messagesList.innerHTML = ''; // Clear the list before appending updated messages
      ...
      messagesList.scrollTop = messagesList.scrollHeight; // Scroll to the bottom after loading messages
    });
    ...
    async function sendMessage(convoId, crushName) {
  ...
  conversationHeading.textContent = `Conversation with: ${crushEmail}`;
